"""
Progressive tennis match prediction system
Uses transformer models to predict match outcomes at different stages
...
# Usage example:
# predictor = main()
# prediction = predictor.predict_progressive_match('Novak Djokovic', 'Rafael Nadal', 'Clay', 5, sets_so_far=[(6,4), (4,6)])
...
class ProgressiveTennisPredictor:
    ...
    def calculate_set_features(self, sets_so_far, is_winner_perspective=True):
        """Extract features from completed sets"""
        if not sets_so_far:
            return [0.0] * 10
        ...
        # Sets won
        sets_won = sum(1 for w,l in sets_so_far if w > l)
    ...
    def load_and_prepare_data(self, csv_file):
        """Load and prepare ATP data with player, surface, and match features"""
    ...
    def calculate_player_stats(self, df):
        """Calculate player statistics by surface, set, and match level"""
    ...
    def get_enhanced_player_features(self, player, surface, best_of):
        ...
        # Experience (log scale works better)
        ...
        ranking_quality = max(0, (200 - avg_ranking) / 200)  # normalize so lower rank = higher score
        features.append(ranking_quality)
        
        # Set and game level features
        ...
        # Versatility (how well-rounded)
        versatility = min(stats['clay_matches'], stats['grass_matches'], stats['hard_matches']) / max(match_count / 3, 1)
        features.extend([overall_winrate, format_specialization, versatility])  # reusing overall_winrate as form proxy
        
        return features
    
    def prepare_progressive_features(self, df, stage='pre_match'):
        """Prepare features prediction stages"""
    ...
    def train_progressive_models(self, df, epochs=150, batch_size=64):
        """Train models for each match stage"""
        ...
        for stage in stages:
            ...
            if len(X) < 100:
                print(f"Not enough data for {stage}: {len(X)} samples, skipping")
                continue
...
def demonstrate_progressive_prediction(predictor):
    """Demo: show predictions at different match stages"""
...
def analyze_prediction_accuracy_improvement(predictor, test_df):
    """Check accuracy improvement by stage"""
...
def test_real_match_prediction(predictor, test_df, num_matches=5):
    """Test on actual match data"""
    
    print("\nTesting on real matches...")
